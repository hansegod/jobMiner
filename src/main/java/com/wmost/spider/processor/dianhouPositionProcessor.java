...
public class dianhouPositionProcessor implements PageProcessor {
    ...
    public static final String URL_MORE = "(.*p=company\\&page=[1-9]{1,3}.*)|(.*company/detail/[0-9]{1,10}.*)";
        ...
        if(page.getUrl().regex(URL_TARGET).match()){
			...
			page.putField(LOG.error_code, page.getStatusCode()+"");
			{
				position[] p = new position[1];
				{
					...
					p[0].company		= page.getHtml().xpath("html/body/div[3]/div/div[1]/div/div[1]/div[2]/h3/span[1]/text()").toString();
